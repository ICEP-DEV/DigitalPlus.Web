...
const MentorsContent = () => {
  ...
  const handleRemoveModule = async (assignModId) => {
    try {
      await axios.delete(
        `https://localhost:7163/api/AssignMod/delete/${assignModId}`
      );
    ...
    try {
      ...
      await Promise.all(
        moduleAssignments.map((assignment) =>
          axios.post(
            "https://localhost:7163/api/AssignMod/AssignModule",
            assignment
          )
        )
      );
      ...
      const response = await axios.get(
        `https://localhost:7163/api/AssignMod/getmodulesBy_MentorId/${selectedMentorId}`
      );
      if (response.data && response.data.length > 0) {
        setAssignedModules(response.data);
        showPopupMessage("Modules assigned successfully!");
  ...
  const handleDeleteMentor = async () => {
    if (mentorToDelete) {
      try {
        await axios.delete(
          `https://localhost:7163/api/DigitalPlusUser/DeleteMentor/${mentorToDelete.mentorId}`
        );
        setMentors(
          mentors.filter(
  ...
  const handleAddMentor = async () => {
    ...
    try {
      ...
      const newMentor = {
        ...
        available:
          mentorForm.available !== undefined ? mentorForm.available : 0,
      ...
      <p>Your account has been created by the administrator. Please use your student email <strong>${
        newMentor.studentEmail
      }</strong> and password <strong>${generatedPassword}</strong> to log in. You can access the platform using the following link:</p>
  ...
  const handleEditMentor = async () => {
    ...
    try {
      ...
      const updatedMentors = mentors.map((mentor) =>
        mentor.mentorId === mentorForm.mentorId
                    ...
                    <td>
                      <Button
                        ...
                        className={`${styles.statusToggleButton} ${
                          mentor.activated ? styles.activate : styles.deactivate
                        }`}
                      >
